...
@dataclass(slots=True)
class GetCurrentWeatherRequestLocationVariant:
    pass

@dataclass(slots=True)
class GetCurrentWeatherRequestLocationCityVariant(GetCurrentWeatherRequestLocationVariant):
    city: str

@dataclass(slots=True)
class GetCurrentWeatherRequestLocationCoordinatesVariant(GetCurrentWeatherRequestLocationVariant):
    coordinates: Coordinates

@dataclass(slots=True)
class GetCurrentWeatherRequest:
    location: GetCurrentWeatherRequestLocationVariant | _UnsetType = UNSET
...
def _from_pb_coordinates(message: weather_pb2.Coordinates) -> Coordinates:
    return Coordinates(
        latitude=message.latitude,
        longitude=message.longitude,
    )

def _to_pb_coordinates(value: Coordinates) -> weather_pb2.Coordinates:
    message = weather_pb2.Coordinates()
    message.latitude = value.latitude
    message.longitude = value.longitude
    return message

def _from_pb_get_current_weather_request(message: weather_pb2.GetCurrentWeatherRequest) -> GetCurrentWeatherRequest:
    location = UNSET
    location_case = message.WhichOneof("location")
    if location_case == "city":
        location = GetCurrentWeatherRequestLocationCityVariant(city=message.city)
    elif location_case == "coordinates":
        location = GetCurrentWeatherRequestLocationCoordinatesVariant(coordinates=_from_pb_coordinates(message.coordinates))
    return GetCurrentWeatherRequest(
        location=location,
    )

def _to_pb_get_current_weather_request(value: GetCurrentWeatherRequest) -> weather_pb2.GetCurrentWeatherRequest:
    message = weather_pb2.GetCurrentWeatherRequest()
    if value.location is not UNSET:
        if isinstance(value.location, GetCurrentWeatherRequestLocationCityVariant):
            message.city = value.location.city
        elif isinstance(value.location, GetCurrentWeatherRequestLocationCoordinatesVariant):
            message.coordinates.CopyFrom(_to_pb_coordinates(value.location.coordinates))
        else:
            raise TypeError("unsupported GetCurrentWeatherRequestLocationVariant variant: " + type(value.location).__name__)

    return message

def _from_pb_get_current_weather_response(message: weather_pb2.GetCurrentWeatherResponse) -> GetCurrentWeatherResponse:
    return GetCurrentWeatherResponse(
        condition=message.condition,
        temperature=message.temperature,
    )

def _to_pb_get_current_weather_response(value: GetCurrentWeatherResponse) -> weather_pb2.GetCurrentWeatherResponse:
    message = weather_pb2.GetCurrentWeatherResponse()
    message.condition = value.condition
    message.temperature = value.temperature
    return message

class WeatherAPIToolHandler(Protocol):
    def get_current_weather(self, ctx: ToolRequestContext, req: GetCurrentWeatherRequest) -> GetCurrentWeatherResponse | Awaitable[GetCurrentWeatherResponse]:
        ...

def register_weather_api_tools(server: mcp.server.lowlevel.Server, impl: WeatherAPIToolHandler, *, namespace: str | None = None) -> None:
    if impl is None:
        raise ValueError("register_weather_api_tools: impl is nil")
    registry = _install_server_handlers(server)
    resolved_namespace = _normalize_namespace(namespace, "weather")
    registry.add_tool(_RegisteredTool(
        name=_tool_name(resolved_namespace, "GetCurrentWeather"),
        title="",
        description="Gets the current weather for a specific location.",
        input_schema_json=WEATHER_API_GET_CURRENT_WEATHER_INPUT_SCHEMA_JSON,
        output_schema_json=WEATHER_API_GET_CURRENT_WEATHER_OUTPUT_SCHEMA_JSON,
        request_type=weather_pb2.GetCurrentWeatherRequest,
        response_type=weather_pb2.GetCurrentWeatherResponse,
        from_pb=_from_pb_get_current_weather_request,
        to_pb=_to_pb_get_current_weather_response,
