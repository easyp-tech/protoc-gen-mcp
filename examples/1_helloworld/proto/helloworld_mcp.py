...
def _from_pb_say_hello_request(message: helloworld_pb2.SayHelloRequest) -> SayHelloRequest:
    return SayHelloRequest(
        name=message.name,
    )

def _to_pb_say_hello_request(value: SayHelloRequest) -> helloworld_pb2.SayHelloRequest:
    message = helloworld_pb2.SayHelloRequest()
    message.name = value.name
    return message

def _from_pb_say_hello_response(message: helloworld_pb2.SayHelloResponse) -> SayHelloResponse:
    return SayHelloResponse(
        message=message.message,
    )

def _to_pb_say_hello_response(value: SayHelloResponse) -> helloworld_pb2.SayHelloResponse:
    message = helloworld_pb2.SayHelloResponse()
    message.message = value.message
    return message

class GreeterAPIToolHandler(Protocol):
    def say_hello(self, ctx: ToolRequestContext, req: SayHelloRequest) -> SayHelloResponse | Awaitable[SayHelloResponse]:
        ...

def register_greeter_api_tools(server: mcp.server.lowlevel.Server, impl: GreeterAPIToolHandler, *, namespace: str | None = None) -> None:
    if impl is None:
        raise ValueError("register_greeter_api_tools: impl is nil")
    registry = _install_server_handlers(server)
    resolved_namespace = _normalize_namespace(namespace, "greeter")
    registry.add_tool(_RegisteredTool(
        name=_tool_name(resolved_namespace, "SayHello"),
        title="",
        description="Returns a friendly greeting for the given name.",
        input_schema_json=GREETER_API_SAY_HELLO_INPUT_SCHEMA_JSON,
        output_schema_json=GREETER_API_SAY_HELLO_OUTPUT_SCHEMA_JSON,
        request_type=helloworld_pb2.SayHelloRequest,
        response_type=helloworld_pb2.SayHelloResponse,
        from_pb=_from_pb_say_hello_request,
        to_pb=_to_pb_say_hello_response,
        handler=impl.say_hello,
        annotations=None,
        icons=None,
        execution=None,
    ))

GREETER_API_SAY_HELLO_INPUT_SCHEMA_JSON = "{\"type\":\"object\",\"properties\":{\"name\":{\"type\":\"string\",\"description\":\"The name of the person to greet.\",\"examples\":[\"Alice\"]}},\"examples\":[{\"name\":\"example\"}],\"required\":[\"name\"],\"additionalProperties\":false}"

GREETER_API_SAY_HELLO_OUTPUT_SCHEMA_JSON = "{\"type\":\"object\",\"properties\":{\"message\":{\"type\":\"string\",\"examples\":[\"example\"]}},\"examples\":[{\"message\":\"example\"}],\"required\":[\"message\"],\"additionalProperties\":false}"
