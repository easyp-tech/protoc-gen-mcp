...
2. **Tool name mangling:** Вариант A — **truncate + SHA-256 hash**. Реализовать в `mcpruntime/options.go` для Go, и аналогично в генерируемом коде для Python/Kotlin/Java/TypeScript.

## Границы scope

- **Must-have (v1):**
  - Функция `MangleToolName(name string, maxLen int) string` в `mcpruntime`
  - Интеграция в `qualifyToolName()` после формирования полного имени
  - Аналогичная логика в генерируемом коде 4 других языков
  - Unit-тесты: короткие имена без изменений, длинные — усечение + hash, детерминизм
  - Обновление golden тестов

- **Deferred (v2):**
  - TaskSupport в Go (ждём Go SDK)
  - Конфигурируемый maxLen через plugin option
  - Warning при генерации если имя будет усечено

## Предположения и открытые вопросы

- [ASSUMPTION: 64 символа — верный лимит] Claude Desktop использует 64, но другие клиенты могут иметь другие лимиты. Hardcoded 64 или параметр?
- [ASSUMPTION: mangling только при превышении] Имена ≤ 64 символов остаются как есть — не хешируем всё подряд.
- [ASSUMPTION: SHA-256 base36 9 символов достаточно для уникальности] 36^9 = ~1.01 × 10^14 — коллизии практически невозможны.

**Открытый вопрос:** нужно ли выдавать warning пользователю когда имя tool'а было mangled? Это помогло бы в отладке.
